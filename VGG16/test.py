...
def test_dfl(model, dataset, batch_size=64, use_gpu=False):
    """
    params model_path: "xxx.pkl"
    params dataset   : type of MyDataset
    params batch_size: int
    ...
    for data, labels in tqdm(dataloader):
        ...
        # Predict
        outputs = model(data)
        outputs = outputs[0] + outputs[1] + 0.1 * outputs[2]
...
if __name__ == "__main__":
    ...
    for i in range(51,100,5):
        ...
        model.load_state_dict(torch.load(path))
        test_dfl(model, dataset, batch_size=BATCH_SIZE, use_gpu=GPU)